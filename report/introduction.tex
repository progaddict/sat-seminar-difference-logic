...
The main difference between linear arithmetic
...
and then introduces a pseudo-variable~$ z $
...
with the pseudo-variable $ z $ in the sense
that introducing this pseudo-variable $ z $
...
\begin{equation}
    \begin{aligned}
        ...
        (\neg ((v-z < -3) \lor (z-v < 3))) \land ((w-z < 4) \lor (z-w < -4)) \\
    \end{aligned}
\end{equation}
...
Let $ p_{mj} = True $ if job $ j $ is scheduled on machine
$ m $ and $ p_{mj} = False $ otherwise.
...
from the $ \mathcal{B} $ set
in Definition~\ref{def:difference-logic}
and $ t_j $ are real variables
from the $ \mathcal{X} $ set
in Definition~\ref{def:difference-logic}
